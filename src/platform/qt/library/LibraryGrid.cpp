...
void LibraryGrid::setShowFilename(bool show) {
	m_showFilename = show;
}
...
void LibraryGrid::addEntries(const QList<LibraryEntry>& items) {
	for (const auto& item : items) addEntry(item);
}

void LibraryGrid::addEntry(const LibraryEntry& item) {
	if (m_items.contains(item.fullpath)) return;
